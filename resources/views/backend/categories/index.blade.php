...
<div class="bg-body-light">
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
        ...
        <form action="{{route('categories.delete')}}" method="POST" id="del_form" form="del_form">
            {{csrf_field()}}
            <button class="btn btn-outline-danger me-1 mb-3" type="button" id="deleteAll"> <i class="fas fa-trash-alt"></i> Delete </button>        
        </form>
        <a href="{{route('categories.create')}}" class="btn btn-outline-success me-1 mb-3">
          ...
          <table class="js-table-checkable table table-hover table-vcenter">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  @if($v->is_special==1)
                  <a class="badge bg-success" href="{{route('categories.special', $v->id)}}">Yes</a>
                  @else
                  <a class="badge bg-warning" href="{{route('categories.special', $v->id)}}">No</a>
                  @endif
                </td>                
                <td>
                  @if($v->display==1)
                  <a class="badge bg-success" href="{{route('categories.display', $v->id)}}">Yes</a>
                  @else
                  <a class="badge bg-warning" href="{{route('categories.display', $v->id)}}">No</a>
                  @endif
                </td>                
                <td class="text-center fs-sm">
                  ...
                  <a class="btn btn-sm btn-alt-secondary" href="{{route('categories.edit', $v->id)}}" data-bs-toggle="tooltip" title="Edit">
