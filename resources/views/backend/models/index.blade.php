...
<div class="bg-body-light">
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
        ...
        <form action="{{route('models.delete')}}" method="POST" id="del_form" form="del_form">
            {{csrf_field()}}
  ...
  @if(Session::has('success'))
    ...
    <div class="block block-rounded">
      <div class="block-header block-header-default">
        <h3 class="block-title">All Models</h3>
          ...
          <table class="js-table-checkable table table-hover table-vcenter">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="d-none d-md-table-cell fs-sm">{{$v->make->name??''}}</td>
                ...
                <td class="text-center fs-sm">
                  <a class="btn btn-sm btn-alt-secondary" href="{{route('models.status', $v->id)}}" data-bs-toggle="tooltip" title="{{($v->is_active==1)?'Un-publish':'Publish'}}">
                    <i class="fa fa-fw {{($v->is_active==1)?'fa-eye-slash':'fa-eye'}}"></i>
                  </a>
                  <a class="btn btn-sm btn-alt-secondary edit-item" data-url="{{route('models.update', $v->id)}}" data-name="{{$v->name}}" data-make="{{$v->make_id}}" href="javascript:;" data-bs-toggle="tooltip" title="Edit">
    ...
    <div class="modal-content">
      <form action="{{route('models.store')}}" method="POST">
        <div class="block block-rounded block-transparent mb-0">
          <div class="block-header block-header-default">
            <h3 class="block-title">Add New Model</h3>
            <div class="block-options">
      ...
      <form id="editForm" method="POST">
        <div class="block block-rounded block-transparent mb-0">
          ...
          <div class="block-content fs-sm">            
            ...
            <div class="form-group pb-1">
                <label for="">Make</label>
                <select class="form-select" name="make_id" required id="edit-make">
                  @foreach($makes as $make)
                  <option value="{{$make->id}}">{{$make->name}}</option>
                  @endforeach
                </select>
