...
<div class="bg-body-light">
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
        ...
        <form action="{{route('salonegoo_faqs.delete')}}" method="POST" id="del_form" form="del_form">
            {{csrf_field()}}
            <button class="btn btn-outline-danger me-1 mb-3" type="button" id="deleteAll"> <i class="fas fa-trash-alt"></i> Delete </button>
        </form>
        <a href="{{route('salonegoo_faqs.create')}}" class="btn btn-outline-success me-1 mb-3">
          ...
          <table class="js-table-checkable table table-hover table-vcenter">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="d-none d-md-table-cell fs-sm">{{($v->category_name)??''}}</td>
                ...
                <td class="text-center fs-sm">
                  <a class="btn btn-sm btn-alt-secondary" href="{{route('salonegoo_faqs.status', $v->id)}}" data-bs-toggle="tooltip" title="{{($v->is_active==1)?'Un-publish':'Publish'}}">
                    <i class="fa fa-fw {{($v->is_active==1)?'fa-eye-slash':'fa-eye'}}"></i>
                  </a>
                  <a class="btn btn-sm btn-alt-secondary" href="{{route('salonegoo_faqs.edit', $v->id)}}" data-bs-toggle="tooltip" title="Edit">
