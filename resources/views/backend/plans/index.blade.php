...
<div class="bg-body-light">
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
        ...
        <form action="{{route('plans.delete')}}" method="POST" id="del_form" form="del_form">
            {{csrf_field()}}
            <button class="btn btn-outline-danger me-1 mb-3" type="button" id="deleteAll"> <i class="fas fa-trash-alt"></i> Delete </button>        
        </form>
        <a href="{{route('plans.create')}}" class="btn btn-outline-success me-1 mb-3">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center fs-sm">
                  <img src="{{$v->icon??asset('placeholder.png')}}" class="img-responsive img-thumbnail" alt="*">
                </td>
                <td class="d-none d-md-table-cell fs-sm">{{$v->name}}</td>
                <td class="d-none d-md-table-cell fs-sm">{{ $v->planType->name??'' }}</td>
                <td class="d-none d-md-table-cell fs-sm">
                  <ul style="list-style: none;padding: 0px">
                        @foreach ($v->getPlanPrice as $price)
                            <li>{{ $price->month }} Month -
                                {{ baseSymbol() }} {{ $price->price }}
                            </li>
                        @endforeach
                    </ul>
                </td>
                <td>
                  @if($v->is_active==1)
                  <a href="{{route('plans.status', $v->id)}}" class="badge bg-success">Published</a>
                  @else
                  <a href="{{route('plans.status', $v->id)}}" class="badge bg-warning">Un-publish</a>
                  @endif
                </td>
                ...
                <td class="text-center fs-sm">
                  {{-- <a class="btn btn-sm btn-alt-secondary" href="{{route('plans.status', $v->id)}}" data-bs-toggle="tooltip" title="{{($v->is_active==1)?'Un-publish':'Publish'}}">
                    <i class="fa fa-fw {{($v->is_active==1)?'fa-eye-slash':'fa-eye'}}"></i>
                  </a> --}}
                  <a class="btn btn-sm btn-alt-secondary" href="{{route('plans.edit', $v->id)}}" data-bs-toggle="tooltip" title="Edit">
