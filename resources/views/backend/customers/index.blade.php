          ...
          <table class="js-table-checkable table table-hover table-vcenter">
            <thead>
              <tr>
                ...
                <th class="d-none d-md-table-cell">Total Ads</th>
                <th class="d-none d-md-table-cell">Money Spent</th>
                <th>Status</th>
                <th>Verified</th>
                <th class="d-none d-sm-table-cell text-center">Registerd At</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="d-none d-md-table-cell fs-sm">
                  {{$v->getFullNameAttribute()}}
                  @if($v->name!=null && $v->name!=$v->getFullNameAttribute())
                    <small>({{$v->name}})</small>
                  @endif
                </td>
                <td class="d-none d-md-table-cell fs-sm">{{$v->email}}</td>
                <td class="d-none d-md-table-cell fs-sm">{{$v->ads->count()}}</td>
                <td class="d-none d-md-table-cell fs-sm">{{formatPrice($v->moneySpent())}}</td>
                <td>
                    @if($v->is_active==1)
                ...
                <td class="text-center fs-sm">   
                  <div class="d-flex">
                    <a class="btn btn-sm btn-alt-secondary" href="{{route('customers.status', $v->id)}}" data-bs-toggle="tooltip" title="{{($v->is_active==1)?'Deactivate':'Activate'}}">
                        <i class="fa fa-fw {{($v->is_active==1)?'fa-eye-slash':'fa-eye'}}"></i>
                    </a>           
                    <a class="btn btn-sm btn-alt-secondary" href="{{route('customers.verify', $v->id)}}" data-bs-toggle="tooltip" title="{{($v->is_verified==1)?'Unverify':'Verify'}}">
                        <i class="fa fa-fw {{($v->is_verified==1)?'fa-ban':'fa-check'}}"></i>
                    </a>
                    <a class="btn btn-sm btn-alt-secondary" href="{{route('customers.edit', $v->id)}}" data-bs-toggle="tooltip" title="Edit Customer">
