...
<div class="bg-body-light">
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
        ...
        <form action="{{route('events-categories.delete')}}" method="POST" id="del_form" form="del_form">
            {{csrf_field()}}
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center fs-sm">
                  <a class="btn btn-sm btn-alt-secondary" href="{{route('events-categories.status', $v->id)}}" data-bs-toggle="tooltip" title="{{($v->is_active==1)?'Un-publish':'Publish'}}">
                    <i class="fa fa-fw {{($v->is_active==1)?'fa-eye-slash':'fa-eye'}}"></i>
                  </a>
                  <a class="btn btn-sm btn-alt-secondary edit-item" data-url="{{route('events-categories.update', $v->id)}}" data-title="{{$v->title}}" data-desc="{{$v->short_description}}" data-order="{{$v->sort_order}}" data-slug="{{$v->slug}}" href="javascript:;" data-bs-toggle="tooltip" title="Edit">
        ...
        <div class="block block-rounded block-transparent mb-0">
          ...
          <div class="block-content fs-sm">
            <div class="form-group pb-1">
              ...
              <input type="text" class="form-control" placeholder="Title" name="title" id="add-title" required>
            </div>
            <div class="form-group pb-1">
              <label for="">Slug <a href="javascript:;" class="generate-slug" data-type="add"><i class="fa fa-refresh"></i></a></label>
              <input type="text" class="form-control" placeholder="Slug" name="slug" id="add-slug" required>
            </div>
            <div class="form-group pb-1">
      ...
      <form id="editForm" method="POST">
        <div class="block block-rounded block-transparent mb-0">
          ...
          <div class="block-content fs-sm">
            ...
            <div class="form-group pb-1">
                <label for="">Slug <a href="javascript:;" class="generate-slug" data-type="edit"><i class="fa fa-refresh"></i></a></label>
                <input type="text" class="form-control" placeholder="Slug" name="slug" id="edit-slug" required>
            </div>
            <div class="form-group pb-1">
                <label for="">Short Description</label>
                <textarea class="form-control" placeholder="Short Description" name="short_description" id="edit-desc"></textarea>
            </div>
            <div class="form-group pb-1">
              <label for="">Sort Order</label>
...
//   $('.image-placeholder').filemanager('image');
    $(document).on('click','.generate-slug',function(){
        $("#"+$(this).data('type')+"-slug").val(convertToSlug($('#'+$(this).data('type')+'-title').val()));
    });
