        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="form-group pb-2">
                    <label for="id{{$rand}}">Slider</label>
                    <select name="components[{{$rand}}][slider][id]" class="form-control input-sm" id="id{{$rand}}">
                        <option value="">Select Slider</option>
                        @foreach(allSliders() as $slider)
                        <option value="{{$slider->id}}" @isset($meta['id']) {{($slider->id==$meta['id'])?'selected':''}} @endisset>{{$slider->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
