...
<div class="bg-body-light">
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
        ...
        <form action="{{route('redirections.delete')}}" method="POST" id="del_form" form="del_form">
            {{csrf_field()}}
  ...
  @if(Session::has('success'))
    ...
    <div class="block block-rounded">
      <div class="block-header block-header-default">
        <h3 class="block-title">All Redirections</h3>
          ...
          <table class="js-table-checkable table table-hover table-vcenter">
            <thead>
              <tr>
                ...
                <th class="d-none d-md-table-cell">Redirect Url</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="d-none d-md-table-cell fs-sm">{{$v->url}}</td>
                <td class="d-none d-md-table-cell fs-sm">{{$v->redirect_url}}</td>
                ...
                <td class="text-center fs-sm">
                  <a class="btn btn-sm btn-alt-secondary" href="{{route('redirections.status', $v->id)}}" data-bs-toggle="tooltip" title="{{($v->is_active==1)?'Un-publish':'Publish'}}">
                    <i class="fa fa-fw {{($v->is_active==1)?'fa-eye-slash':'fa-eye'}}"></i>
                  </a>
                  <a class="btn btn-sm btn-alt-secondary edit-redirection" data-url="{{route('redirections.update', $v->id)}}" data-curl="{{$v->url}}" data-redirect="{{$v->redirect_url}}" href="javascript:;" data-bs-toggle="tooltip" title="Edit">
    ...
    <div class="modal-content">
      <form action="{{route('redirections.store')}}" method="POST">
        <div class="block block-rounded block-transparent mb-0">
          <div class="block-header block-header-default">
            <h3 class="block-title">Add New Redirection</h3>
            <div class="block-options">
          ...
          <div class="block-content fs-sm">
            <div class="form-group pb-2">
                <label>URL (Redirect from) {{url('/')}}/</label>
                <input type="text" class="form-control" name="url" placeholder="Enter URL" required>
            </div>
            <div class="form-group">
                <label>{{url('/')}}/</label>
                <input type="text" class="form-control" name="redirect_url" placeholder="Redirect slug" required>
            </div>
          </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <form id="editForm" method="POST">
        <div class="block block-rounded block-transparent mb-0">
          <div class="block-header block-header-default">
            <h3 class="block-title">Edit Redirection</h3>
            <div class="block-options">
              <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                <i class="fa fa-fw fa-times"></i>
              </button>
          ...
          <div class="block-content fs-sm">
            <div class="form-group pb-2">
                <label>URL (Redirect from) {{url('/')}}/</label>
                <input type="text" class="form-control" name="url" placeholder="Enter URL" required id="edit-curl">
            </div>
            <div class="form-group">
                <label>{{url('/')}}/</label>
                <input type="text" class="form-control" name="redirect_url" placeholder="Redirect slug" required id="edit-redirect">
            </div>
          </div>
          <div class="block-content block-content-full text-end bg-body">
            @csrf
...
<script>
  ...
  $('.edit-redirection').click(function(){
    $("#editForm").attr('action', $(this).data('url'));
    $("#edit-curl").val($(this).data('curl'));
    $("#edit-redirect").val($(this).data('redirect'));
