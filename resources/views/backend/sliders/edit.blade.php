...
<form action="{{route('sliders.update',$data['id'])}}" method="POST">
    ...
    <div class="bg-body-light">
        <div class="content content-full">
          ...
          <div class="row">
            <div class="col-md-5">
                <input type="text" name="name" class="form-control light-fields" placeholder="Name" required value="{{ ($data->name)??'' }}">
                @csrf
            </div>
        </div>
        </div>
                ...
                <div class="block block-rounded mt-3 pb-3">
                    ...
                    <div class="block-content">
                        <div class="row" id="sliders">
                            @if(isset($sliders))
                                @foreach($sliders['image'] as $k => $slider)
                                    <div class="block block-rounded mt-1 pb-1 removeDiv">
                                        <div class="block-header block-header-default">
                                            ...
                                            <a data-id="edit_slider{{ $k }}" class="edit_slider"><i class="fa fa-minus"></i></a>
                                            <span class="divider" style="margin: 0 5px;"></span>
                                            <a href="#" class="remove_slider"><i class="fa fa-times"></i></a>
                                        </div>
                                        <div class="block-content" id="edit_slider{{ $k }}">
                                            <div class="row">
                                                <div class="form-group text-center m-b-20">
                                                    <div class="image-placeholder" id="wfm" data-input="hidden-{{ $k }}" data-preview="holder-{{ $k }}">
                                                        <div class="imagepreview" id="holder-{{ $k }}">
                                                            @if(isset($slider))
                                                                <img src="{{ $slider }}">
                                                            @else
                                                    ...
                                                    <input type="hidden" name="slider[image][]" id="hidden-{{ $k }}">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
