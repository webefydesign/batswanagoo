...
@endsection
@section('content')
<form action="{{route('plan-types.update', $data['id'])}}" method="POST">
                        ...
                        <div class="row" id="allPoints">
                            ...
                            <div class="col-sm-12">
                                <div class="row mt-3">
                                    <div class="col-sm-9">
                                        <div class="form-group">
                                            <input type="text" class="form-control input-sm" name="points[]" placeholder="Point" value="{{$point}}">
                                        </div>
                                    </div>
                                    <div class="col-sm-3">
                ...
                <div class="block block-rounded mt-3 pb-3">
                    ...
                    <div class="block-content">
                        <select multiple="multiple" id="cat-select" name="categories[]">
                            ...
                            <option value="{{$cat->id}}" {{(isset($data->categories) && count($data->categories) > 0 && in_array($cat->id, $data->categories->pluck('category_id')->toArray()))?'selected':''}}>{{$cat->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
