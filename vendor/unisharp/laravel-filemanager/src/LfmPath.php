...
class LfmPath
{
    ...
    public function upload($file)
    {
        ...
        /* Watermarking disabled - commented out, not removed, in case it's
           needed again later.
        try {
            // Check if the file is an image
            if (in_array($file->getMimeType(), ['image/jpeg', 'image/png', 'image/gif'])) {
                $watermarkUrl = (getConfigurations()['watermark']) ?? null;
        
                // Convert URL to a local path if the watermark is stored locally
                $watermarkPath = null;
                if ($watermarkUrl) {
                    $watermarkPath = public_path(parse_url($watermarkUrl, PHP_URL_PATH));
                }
                ...
                if ($watermarkPath && file_exists($watermarkPath)) {
                    ...
                    if ($sourceImage) {
                        ...
                        // Calculate position for the watermark (bottom-right with 10px padding)
                        ...
                        // Save the new image
                        if ($mimeType === 'image/jpeg') {
                            imagejpeg($sourceImage, $new_file_path);
                        } elseif ($mimeType === 'image/png') {
                            imagepng($sourceImage, $new_file_path);
                        } elseif ($mimeType === 'image/gif') {
                            imagegif($sourceImage, $new_file_path);
                        }
        
                        // Destroy resources to free memory
                        imagedestroy($sourceImage);
                        imagedestroy($watermark);
                    } else {
                        throw new \Exception("Could not process the image.");
                    }
                } else {
                    // If no watermark, save the file as is
                    $this->setName($new_file_name)->storage->save($file);
                }
            } else {
                // If not an image, save the file as is
                $this->setName($new_file_name)->storage->save($file);
            }
        } catch (\Exception $e) {
